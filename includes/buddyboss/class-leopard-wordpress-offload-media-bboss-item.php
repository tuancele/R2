...
class Leopard_Wordpress_Offload_Media_BBoss_Item extends Leopard_Wordpress_Offload_Media_Item {
	/**
	...
	public static function create_from_source_id( $source_id, $options = array() ) {
		...
		if ( empty( $file_paths ) ) {
			return new WP_Error(
				'exception',
				__( 'No local files found in ' . __FUNCTION__, 'leopard-wordpress-offload-media' )
			);
		}

		$file_path = static::remove_size_from_filename( $file_paths[ Leopard_Wordpress_Offload_Media_Item::primary_object_key() ] );
	...
	public function get_new_item_prefix( $use_object_versioning = true ) {
		...
		$prefix = leopard_offload_media_get_object_prefix();
		// Buddy Boss specific prefix
		$buddy_boss_prefix = sprintf( static::$prefix_pattern, $this->source_id() );
		$prefix            .= Leopard_Wordpress_Offload_Media_Utils::trailingslash_prefix( $buddy_boss_prefix );
	...
	public static function get_local_files( $source_id ) {
		...
		foreach ( $files as $file ) {
			...
			$file_name = Leopard_Wordpress_Offload_Media_Utils::unleadingslashit( $file_name );

			if ( false !== strpos( $base_name, '-bp-cover-image' ) ) {
				$result[ Leopard_Wordpress_Offload_Media_Item::primary_object_key() ] = $file_name;
			}
	...
	public static function count_items( $skip_transient, $force ) {
		...
		$transient_key = 'leopard_' . get_current_blog_id() . '_attachment_counts_' . static::$source_type;
