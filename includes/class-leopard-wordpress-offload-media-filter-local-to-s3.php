...
class Leopard_Wordpress_Offload_Media_Filter_Local_To_S3 extends Leopard_Wordpress_Offload_Media_Filter {
	...
	protected function init() {
		...
		//Leopard_Wordpress_Offload_Media_Replace_Webp_Content
	...
	public function filter_post_webp( $content ) {
		...
		if($enable_webp && count($cloud_urls) > 0){
			foreach($cloud_urls as $post_id => $url){
				$key = Leopard_Wordpress_Offload_Media_Utils::get_key_from_url($url, false);
				$replace = Leopard_Wordpress_Offload_Media_Utils::get_webp_url_with_key($post_id, $key);
				
				if(!empty($replace)){
					$replace = leopard_wordpress_offload_media_get_real_url($replace);
					$replace = leopard_wordpress_offload_media_s3_to_cloudfront_url($replace);
	...
	public function url_needs_replacing( $url ) {
		return leopard_offload_media_url_needs_replacing($url);
	}
	...
	private function get_bare_upload_base_urls() {
		return leopard_offload_media_get_bare_upload_base_urls();
	}
