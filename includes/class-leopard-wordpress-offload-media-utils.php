...
if ( ! class_exists( 'Leopard_Wordpress_Offload_Media_Utils' ) ) {
	...
	class Leopard_Wordpress_Offload_Media_Utils {
		...
		public static function remove_size_from_filename( $url, $remove_extension = false ) {
			...
			$url = apply_filters( 'leopard_remove_size_from_filename', $url );
		...
		public static function fullsize_paths( $paths ) {
			if ( is_array( $paths ) && ! empty( $paths ) ) {
				return array_values( array_unique( array_intersect_key( $paths, array_flip( array( Leopard_Wordpress_Offload_Media_Item::primary_object_key(), 'file', 'full-orig', 'original_image' ) ) ) ) );
			} else {
				return array();
			}
		}
