<?php
class Leopard_Wordpress_Offload_Media_Library_Item extends Leopard_Wordpress_Offload_Media_Item {
	/**
	 * Source type name
	 *
	 * @var string
	...
	public static function create_from_source_id( $source_id, $options = array() ) {
		if ( empty( $source_id ) ) {
			return new WP_Error(
				'exception',
				__( 'Empty Attachment ID passed to ' . __FUNCTION__, 'leopard-wordpress-offload-media' )
			);
		}

		$default_options = array(
			'originator'            => Leopard_Wordpress_Offload_Media_Item::ORIGINATORS['standard'],
			'is_verified'           => true,
		...
		if ( ! in_array( $options['originator'], self::ORIGINATORS ) ) {
			return new WP_Error(
				'exception',
				__( 'Invalid Originator passed to ' . __FUNCTION__, 'leopard-wordpress-offload-media' )
			);
		}
		...
		if ( ! is_string( $attached_file_meta ) ) {
			return new WP_Error(
				'exception',
				sprintf( __( 'Media Library item with ID %d has damaged meta data', 'leopard-wordpress-offload-media' ), $source_id )
			);
		}
		...
		if ( empty( $source_path ) ) {
			return new WP_Error(
				'exception',
				sprintf( __( 'Media Library item with ID %d does not have a valid file path', 'leopard-wordpress-offload-media' ), $source_id )
			);
		}
		...
		$file_paths = Leopard_Wordpress_Offload_Media_Utils::get_attachment_file_paths( $source_id, false, $attachment_metadata );
	...
	public static function get_by_source_id( $source_id ) {
		$leopard_item = parent::get_by_source_id( $source_id );

		if ( ! $leopard_item ) {
			$provider_object = static::_legacy_get_attachment_provider_info( $source_id );

			if ( is_array( $provider_object ) ) {
				$leopard_item = static::_legacy_provider_info_to_item( $source_id, $provider_object );
			}
		}
	...
	public function full_source_paths() {
		return array_intersect_key( Leopard_Wordpress_Offload_Media_Utils::get_attachment_file_paths( $this->source_id(), false ), $this->objects() );
	}
	...
	public static function count_items( $skip_transient = false, $force = false ) {
		...
		$transient_key = 'leopard_' . get_current_blog_id() . '_attachment_counts';
		...
		if ( $force || $skip_transient || false === ( $result = get_site_transient( $transient_key ) ) ) {
			...
			$local_removed = leopard_offload_media_count_items_local_removed(static::$source_type);

			// Copy to server from S3
			$copy_from_cloud = leopard_offload_media_count_items_downloaded_to_server_from_cloud(static::$source_type);

			// Remove from S3
			$cloud_removed = leopard_offload_media_count_items_remove_file_cloud(static::$source_type);
			...
			set_site_transient( $transient_key, $result, 1 * MINUTE_IN_SECONDS );
	...
	public static function get_missing_source_ids( $upper_bound, $limit, $count = false ) {
		 ...
		 * @param array
		 */
		$ignored_mime_types = apply_filters( 'leopard_ignored_mime_types', array() );
	...
	public function offload_duplicate_items() {
		...
		foreach ( $results as $result ) {
			$leopard_item = new Leopard_Wordpress_Offload_Media_Item(
				$this->provider(),
				$this->region(),
				$this->bucket(),
				$this->path(),
			...
			$leopard_item->save();
			$leopard_item->duplicate_filesize_total( $this->source_id() );
		}
	}
	...
	public static function admin_link( $error ) {
		return (object) array(
			...
			'text' => __( 'Edit', 'leopard-wordpress-offload-media' ),
		);
	}
	...
	/**
	 * Cleanup filesize and leopard_filesize_total metadata on the underlying media library item
	...
	public function update_filesize_after_download_local() {
		...
		delete_post_meta( $this->source_id(), 'leopard_filesize_total' );
	}

	/**
	 * Duplicate 'leopard_filesize_total' meta if it exists for an attachment.
	 *
	 * @param int $attachment_id
	 */
	public function duplicate_filesize_total( $attachment_id ) {
		if ( ! ( $filesize = get_post_meta( $attachment_id, 'leopard_filesize_total', true ) ) ) {
			// No filesize to duplicate.
			return;
		}

		update_post_meta( $this->source_id(), 'leopard_filesize_total', $filesize );
	}
	 ...
	 *
	 * @params Item  $leopard_item
	 * @params array $paths
	 */
	public function remove_duplicate_paths( Leopard_Wordpress_Offload_Media_Item $leopard_item, $paths ) {
		$full_size_paths        = Leopard_Wordpress_Offload_Media_Utils::fullsize_paths( $leopard_item->full_source_paths() );
		$leopard_items_with_paths = static::get_by_source_path( $full_size_paths, array( $leopard_item->source_id() ), false );

		$duplicate_paths = array();

		foreach ( $leopard_items_with_paths as $leopard_item_with_path ) {
			/* @var Media_Library_Item $leopard_item_with_path */
			$duplicate_paths += array_values( Leopard_Wordpress_Offload_Media_Utils::get_attachment_file_paths( $leopard_item_with_path->source_id(), false, false, true ) );
		}
	...
	private static function _legacy_get_attachment_provider_info( $post_id ) {
		...
		$provider_object = apply_filters( 'leopard_get_attachment_s3_info', $provider_object, $post_id ); // Backwards compatibility

		return apply_filters( 'leopard_get_attachment_provider_info', $provider_object, $post_id );
	}
