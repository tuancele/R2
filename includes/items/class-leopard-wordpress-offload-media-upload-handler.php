		...
		try {
			foreach ( $manifest->objects as $object_key => &$object ) {
				...
				try {
					...
					if(!empty($result)){
						$pos = strpos($args['SourceFile'], $leopard_item->source_path());
						if ($pos !== false) {
							$urlOffloaded = $result;
						}
					...
					$error_msg = sprintf( __( 'Error offloading %1$s to provider: %2$s', 'leopard-wordpress-offload-media' ), $args['SourceFile'], $e->getMessage() );
					$object['upload_result']['status']  = self::STATUS_FAILED;
					$object['upload_result']['message'] = $error_msg;
				}
			}
