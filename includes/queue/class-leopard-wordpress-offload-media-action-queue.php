 ...
 * @version 1.0.31
 */
class Leopard_Wordpress_Offload_Media_Action_Queue implements Leopard_Wordpress_Offload_Media_Queue_Interface {
	 ...
	 * @return Leopard_Wordpress_Offload_Media_DateTime|null The date and time for the next occurrence, or null if there is no pending, scheduled action for the given hook.
	 */
	public function get_next( $hook, $args = null, $group = '' ) {
		...
		if ( $next_timestamp ) {
			return new Leopard_Wordpress_Offload_Media_DateTime( "@{$next_timestamp}", new DateTimeZone( 'UTC' ) );
		}
