...
abstract class ActionScheduler_Logger {
	...
	public function log_stored_action( $action_id ) {
		$this->log( $action_id, __( 'action created', 'leopard-wordpress-offload-media' ) );
	}

	public function log_canceled_action( $action_id ) {
		$this->log( $action_id, __( 'action canceled', 'leopard-wordpress-offload-media' ) );
	}

	public function log_started_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action started via %s', 'leopard-wordpress-offload-media' ), $context );
		} else {
			$message = __( 'action started', 'leopard-wordpress-offload-media' );
		}
	...
	public function log_completed_action( $action_id, $action = NULL, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action complete via %s', 'leopard-wordpress-offload-media' ), $context );
		} else {
			$message = __( 'action complete', 'leopard-wordpress-offload-media' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_action( $action_id, Exception $exception, $context = '' ) {
		if ( ! empty( $context ) ) {
			...
			$message = sprintf( __( 'action failed via %1$s: %2$s', 'leopard-wordpress-offload-media' ), $context, $exception->getMessage() );
		} else {
			/* translators: %s: exception message */
			$message = sprintf( __( 'action failed: %s', 'leopard-wordpress-offload-media' ), $exception->getMessage() );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_timed_out_action( $action_id, $timeout ) {
		...
		$this->log( $action_id, sprintf( __( 'action timed out after %s seconds', 'leopard-wordpress-offload-media' ), $timeout ) );
	}

	public function log_unexpected_shutdown( $action_id, $error ) {
		if ( ! empty( $error ) ) {
			/* translators: 1: error message 2: filename 3: line */
			$this->log( $action_id, sprintf( __( 'unexpected shutdown: PHP Fatal error %1$s in %2$s on line %3$s', 'leopard-wordpress-offload-media' ), $error['message'], $error['file'], $error['line'] ) );
		}
	}

	public function log_reset_action( $action_id ) {
		$this->log( $action_id, __( 'action reset', 'leopard-wordpress-offload-media' ) );
	}

	public function log_ignored_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( __( 'action ignored via %s', 'leopard-wordpress-offload-media' ), $context );
		} else {
			$message = __( 'action ignored', 'leopard-wordpress-offload-media' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_fetch_action( $action_id, Exception $exception = NULL ) {
		...
		if ( ! is_null( $exception ) ) {
			/* translators: %s: exception message */
			$log_message = sprintf( __( 'There was a failure fetching this action: %s', 'leopard-wordpress-offload-media' ), $exception->getMessage() );
		} else {
			$log_message = __( 'There was a failure fetching this action', 'leopard-wordpress-offload-media' );
		}
	...
	public function log_failed_schedule_next_instance( $action_id, Exception $exception ) {
		/* translators: %s: exception message */
		$this->log( $action_id, sprintf( __( 'There was a failure scheduling the next instance of this action: %s', 'leopard-wordpress-offload-media' ), $exception->getMessage() ) );
	}
